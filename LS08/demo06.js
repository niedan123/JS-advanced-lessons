//函数对象属性值arguments 实参集合(类似 数组的一个对象)
var foo = function(a,b){
    console.log(arguments);//类似数组的一个对象
    //arguments 是Symbo类型，独一无二的
    //console.log(arguments === test.arguments);//false
    console.log(arguments.length);//实参个数
    var args = Array.prototype.splice.call(arguments,0);//类数组
    console.log(args);
}
foo(1,2,3,4);
...
checkVarCount(1,2);//no
checkVarCount(1);//success
...
test();//如果函数是从 JavaScript 程序的顶层调用的，则 caller 包含 null。
function testOuter(){
    test();
}
...
//函数对象属性之 construction 获取创建某个对象的构造函数。可以用来判断对象是哪一类
...
var emptyObject = {};
//创建一个新函数，该函数使用12和“a”参数作为第一个和第二个参数。 
var displayArgs2 = displayArgs.bind(emptyObject, 12, "a");
// 调用新函数。“b”和“c”参数用作第三和第四个参数。 
